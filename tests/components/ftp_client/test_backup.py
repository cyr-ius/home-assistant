"""Test the backups for FTP Client."""
...
from homeassistant.components.ftp_client.backup import (
    async_register_backup_agents_listener,
)
from homeassistant.components.ftp_client.const import (
    DATA_BACKUP_AGENT_LISTENERS,
    DOMAIN,
)
from homeassistant.components.ftp_client.helpers import CannotConnect
    ...
    assert response["result"]["backups"] == [
        {
            "addons": [],
            "agents": {
                "ftp_client.01JKXV07ASC62D620DGYNG2R8H": {
                    "protected": False,
                    "size": 34519040,
                }
            },
...
async def test_agents_delete(
    ...
    assert response["success"]
    assert response["result"] == {"agent_errors": {}}
    assert ftp_client.client.remove.call_count == 2
...
async def test_agents_upload(
    ...
    assert ftp_client.async_connect.call_count == 2
...
async def test_error_on_agents_download(
    ...
    client = await hass_client()
    backup_id = BACKUP_METADATA["backup_id"]
    ftp_client.client.list.return_value = []
...
async def test_error_download_cannot_connect(
    ...
    with patch(
        "homeassistant.components.ftp_client.backup.FTPDriveBackupAgent._find_backup_by_id",
        ...
        client = await hass_client()
        backup_id = BACKUP_METADATA["backup_id"]
        ftp_client.async_connect.side_effect = CannotConnect()
...
@pytest.mark.parametrize(
    ...
    [
        (
            AIOFTPException(500, "Unknown path"),
            "Backup operation failed: (500, 'Unknown path')",
        )
    ],
)
async def test_delete_error(
    ...
    ftp_client.client.remove.side_effect = side_effect
